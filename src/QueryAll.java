...
*/
        	...
        	String productId = new String(value.getValue(Bytes.toBytes("h"), Bytes.toBytes("ProductID")));
        	String testId = new String(value.getValue(Bytes.toBytes("h"), Bytes.toBytes("TestID")));
        	NavigableMap<byte[], byte[]> measureFamily = value.getFamilyMap(Bytes.toBytes("h"));
        	
        	text.set(productId + "-" + testId);
	...
	public static void main(String[] args) {
		...
		try {
			// Criteria
				...
				conf.set("mapred.tasktracker.map.tasks.maximum", args[1]);
				conf.set("mapred.map.tasks.speculative.execution", "false");
				Job job = new Job(conf,"Product-Test-Value-Outcome-Query for test_v3 " );				
				job.setJarByClass(QueryAll.class);     // class that contains mapper and reducer							
								
				Scan scan = new Scan();
				scan.setCaching(Integer.parseInt(args[0]));        // 1 is the default in Scan, which will be bad for MapReduce jobs
