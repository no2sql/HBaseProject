...
public class BulkLoaderLive extends Configured implements Tool {
	...
	private static String tableName = "test_skinny_live";
	@Override
	public int run(String[] args) throws Exception {
		...
		try {
			...
			*/
			...
			job.setJarByClass(BulkLoaderLive.class);
	...
	public static void main(String[] args) {
		...
		try {
			...
			ToolRunner.run(new BulkLoaderLive(), args);
	...
	throws IOException, ParseException {
	  try {
		  ...
		  if (useSalt){
			  splits = new byte[(NUMBER_OF_NODES-2)*3][];			  
