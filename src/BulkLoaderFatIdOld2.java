...
public class BulkLoaderFatIdOld2 extends Configured implements Tool {
	...
	static class BulkLoaderByFileMapper extends Mapper<LongWritable, Text, ImmutableBytesWritable, Put>  {
		...
		public void map(LongWritable row, Text value, Context context) throws IOException {
			...
			context.getCounter("camstar", "useSalt " + useSalt).increment(1l);
	...
	{				
		try {
			...
			put.add(Bytes.toBytes("h"), Bytes.add(colPrefix, Bytes.toBytes('g')),
					Bytes.toBytes(MyCache.getInstance().get("g", unitReport.getTestRun().getGrade())));
			if (parentTestRun == null)
			...
			put.add(Bytes.toBytes("h"), Bytes.add(colPrefix, Bytes.toBytes('s')),
					Bytes.toBytes(MyCache.getInstance().get("s", unitReport.getProduct().getSerialNo())));
			if (parentTestRun == null)
	...
	private static String tableName = "test_fat_id";
	@Override
	public int run(String[] args) throws Exception {
		...
		try {
			...
			*/
			...
			job.setJarByClass(BulkLoaderFatIdOld2.class);
	...
	public static void main(String[] args) {
		...
		try {
			...
			ToolRunner.run(new BulkLoaderFatIdOld2(), args);
	...
	throws IOException, ParseException {
	  try {
		  ...
		  if (useSalt){
			  splits = new byte[NUMBER_OF_NODES-2][];			  
		  ...
		  else
		  {
			  ...
			  Date date = df.parse("2011-05-13T00:00:00 +0000");
			  cal.setTime(date);
			  splits[i++] = Bytes.add(Bytes.toBytes('P'),Bytes.toBytes(cal.getTimeInMillis()));
