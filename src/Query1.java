...
public class Query1 {
	...
	static class Mapper1 extends TableMapper<Text, Text> {
		...
		public void map(ImmutableBytesWritable row, Result value, Context context) throws IOException {         
			...
			*/
			...
			byte salt = row.get()[0];
			byte[] startTimeBytes = Arrays.copyOfRange(row.get(), Bytes.SIZEOF_BYTE+Bytes.SIZEOF_INT+Bytes.SIZEOF_SHORT, Bytes.SIZEOF_BYTE+Bytes.SIZEOF_INT+Bytes.SIZEOF_SHORT+Bytes.SIZEOF_LONG);
	...
	public static void main(String[] args) {
		...
		try {
			...
			//{
			        //byte salt = 1;
					Job job = new Job(conf,"Kevin Query1  "/* + salt*/);
					//jobList.add(job);
					job.setJarByClass(Query1.class);     // class that contains mapper and reducer					
					Scan scan = new Scan();
