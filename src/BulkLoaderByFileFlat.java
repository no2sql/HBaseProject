...
public class BulkLoaderByFileFlat extends Configured implements Tool {
	...
	{				
		try {
			...
			if (parentTestRun != null)
				colPrefix = String.valueOf(testRun.getTestId())+",";
			
			// put column family -header
			if (testRun.getEndTime() == null)
				put.add(Bytes.toBytes("h"), Bytes.toBytes(colPrefix+"end"),
						Bytes.toBytes(testRun.getEndTime()));
			put.add(Bytes.toBytes("h"), Bytes.toBytes(colPrefix+"gr"),
					Bytes.toBytes(unitReport.getTestRun().getGrade()));
			if (parentTestRun == null)
			put.add(Bytes.toBytes("h"), Bytes.toBytes(colPrefix+"f"),
					Bytes.toBytes(unitReport.getFilename()));
			if (parentTestRun == null)
			put.add(Bytes.toBytes("h"), Bytes.toBytes(colPrefix+"sn"),
					Bytes.toBytes(unitReport.getProduct().getSerialNo()));
			if (parentTestRun == null)
			put.add(Bytes.toBytes("h"), Bytes.toBytes(colPrefix+"pid"),
					Bytes.toBytes(unitReport.getProduct().getPartNo()));
			if (parentTestRun == null)
			put.add(Bytes.toBytes("h"), Bytes.toBytes(colPrefix+"oid"),
					Bytes.toBytes(unitReport.getOperator().getName()));
			put.add(Bytes.toBytes("h"), Bytes.toBytes(colPrefix+"tid"),
					Bytes.toBytes(testRun.getName()));
			if (unitReport.getVersion() != null)
				put.add(Bytes.toBytes("h"), Bytes.toBytes(colPrefix+"vid"),
						Bytes.toBytes(unitReport.getVersion()));
			if (unitReport.getQuantity() != null)
				put.add(Bytes.toBytes("h"), Bytes.toBytes(colPrefix+"qid"),
				...
				for (Object measure : testRun.getResult().getValueDoubleOrValueIntegerOrValueString())
				{
					...
					if (value.getNameString() != null)
					{
						if (value.getLslString() != null)
							put.add(Bytes.toBytes("m"), Bytes.toBytes(colPrefix+value.getNameString() + "-LSL"),
									Bytes.toBytes(value.getLslString()));
						if (value.getUslString() != null)
							put.add(Bytes.toBytes("m"), Bytes.toBytes(colPrefix+value.getNameString() + "-USL"),
									Bytes.toBytes(value.getUslString()));
						if (value.getNameString() != null)
							put.add(Bytes.toBytes("m"), Bytes.toBytes(colPrefix+value.getNameString() + "-mn"),
									Bytes.toBytes(value.getNameString()));
						if (value.getUnitString() != null)
							put.add(Bytes.toBytes("m"), Bytes.toBytes(colPrefix+value.getNameString() + "-mu"),
									Bytes.toBytes(value.getUnitString()));
						if (value.getGradeString() != null)
							put.add(Bytes.toBytes("h"), Bytes.toBytes(colPrefix+value.getNameString() + "-gr"),
									Bytes.toBytes(value.getGradeString()));
						if (value.getValueString() != null)
							put.add(Bytes.toBytes("m"), Bytes.toBytes(colPrefix+value.getNameString()),
			...
			if (parentTestRun == null)
			{
				for (Object property : unitReport.getProperty().getValueDoubleOrValueIntegerOrValueString())
				{
					...
					if (value.getLslString() != null)
						put.add(Bytes.toBytes("cp"), Bytes.toBytes(colPrefix+value.getNameString() + "-LSL"),
								Bytes.toBytes(value.getLslString()));
					if (value.getUslString() != null)
						put.add(Bytes.toBytes("cp"), Bytes.toBytes(colPrefix+value.getNameString() + "-USL"),
								Bytes.toBytes(value.getUslString()));
					if (value.getNameString() != null)
						put.add(Bytes.toBytes("cp"), Bytes.toBytes(colPrefix+value.getNameString() + "-mn"),
								Bytes.toBytes(value.getNameString()));
					if (value.getUnitString() != null)
						put.add(Bytes.toBytes("cp"), Bytes.toBytes(colPrefix+value.getNameString() + "-mu"),
								Bytes.toBytes(value.getUnitString()));
					if (value.getValueString() != null)
						put.add(Bytes.toBytes("cp"), Bytes.toBytes(colPrefix+value.getNameString()),
								Bytes.toBytes(value.getValueString()));
				}
			}
			...
			if (parentTestRun != null) {
				put.add(Bytes.toBytes("p"), Bytes.toBytes(colPrefix+","+parentTestRun.getTestId()),
			...
			if (testRun.getSubtests() != null)
			{
				for (TestRun subtest: testRun.getSubtests())
				{
					...
					put.add(Bytes.toBytes("c"), Bytes.toBytes(colPrefix+","+subtestId), Bytes.toBytes(subtest.getGrade()));
				}
			}
	...
	public int run(String[] args) throws Exception {
		...
		try {
			...
			*/
			...
			job = new Job(conf);
			job.setJobName("Bulk loader by file test_flat_v2");			

			job.setMapOutputKeyClass(ImmutableBytesWritable.class);
			job.setMapOutputValueClass(Put.class);

			job.setMapperClass(BulkLoaderByFileMapper.class);
			job.setJarByClass(BulkLoaderByFileFlat.class);
			
			job.setInputFormatClass(XmlInputFormat.class);		    
			FileInputFormat.setInputPaths(job, "/xml/input");
		
			job.setOutputFormatClass(HFileOutputFormat.class);
			HFileOutputFormat.setOutputPath(job, new Path("/import-data-file-flat"));
	        ...
	        HFileOutputFormat.configureIncrementalLoad(job, new HTable(hConfig, "test_flat_v2"));
	...
	public static void main(String[] args) {
		...
		try {
			...
			ToolRunner.run(new BulkLoaderByFileFlat(), args);
	...
	throws IOException, ParseException {
	  try {
		  ...
		  try{
			  admin.disableTable(Bytes.toBytes("test_flat_v2"));
			  admin.deleteTable(Bytes.toBytes("test_flat_v2"));
		  }
		  catch (Exception e) {
			  e.printStackTrace();
		  ...
		  HTableDescriptor table = new HTableDescriptor(Bytes.toBytes("test_flat_v2"));
		  ...
		  byte[][] splits = new byte[17*NUMBER_OF_NODES][];
		  int i = 0;
		  ...
		  {		  
			  ...
			  cal.setTime(date);
			  splits[i++] = Bytes.add(Bytes.toBytes(salt), Bytes.add(Bytes.toBytes('P'),Bytes.toBytes(cal.getTimeInMillis())));
