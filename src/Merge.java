...
public class Merge extends Configured implements Tool {
  ...
  private int actualMerge() throws Exception {
  {
  ...
  private void mergeTwoRegions() throws IOException {
    ...
    LOG.info("cells1 is " + cells1);
    if (cells1 != null)
    {
    	LOG.info("cells1 size is " + cells1.size());
    	LOG.info("get is " + cells1.get(0));
    }
    HRegionInfo info1 = Writables.getHRegionInfo((cells1 == null)? null: cells1.get(0).getValue());
    if (info1== null) {
  ...
  public int doMerge(byte[] tableName, byte[] region1, byte[] region2) throws Exception
  {
